# Pixelmoebe

Pixelmoebe is a simple Pixelart-editor made from scratch, only using the Java-Swing-Framework. It only contains basic functions such as a color-mixer, Layer and Version System. The only goal of Pixelmoebe is to demonstrate the simplicity of Java-App-Development. 

## **[CLICK TO DOWNLOAD LATEST .EXE](https://minhaskamal.github.io/DownGit/#/home?url=https://github.com/damoebe/Pixelmoebe/blob/master/pixelmoebe_V0.1.exe)**
The exe file is not signed. This means it will be blocked by your computer if you run it. You can bypass this by clicking on "more infos" and "run anyway". This is not a virus!
> [!WARNING]
> You will need an **OpenJDK 21+** to run this!\
> If you don't install it you will get an error-message when you lauch the exe.

> [!IMPORTANT]
> The link above is sometimes **blocked by google**, you can still download the exe manually.\
> If you are planing to make real art and shi don't touch this! 


